...
export default {
  ...
  created() {
    this.$http.get("http://localhost:3000/cate/list").then(res => {
      console.log(res);
      this.data = res.data;
    });
  },
  methods: {
    ...
    deviceTree(Num) {
      ...
      this.$http.post("http://localhost:3000/cate/device", data).then(res => {
        this.data = res.data;
      });
    },
    confirm() {
    ...
    removeit() {
      this.$http
        .delete("http://localhost:3000/cate/data/" + this.Id)
        .then(res => {
          this.reGetData();
          this.modal = false;
          this.$Notice.success({
    ...
    addRootNode() {
      ...
      this.$http.post("http://localhost:3000/cate/data", data).then(res => {
        this.reGetData();
        this.modal = false;
        this.$Notice.success({
          title: "通知",
